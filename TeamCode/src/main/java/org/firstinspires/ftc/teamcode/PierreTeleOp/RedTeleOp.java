...
public class RedTeleOp extends LinearOpMode {
    ...
    private boolean encoderfunctionoff = false;
    private boolean intakefunctionon = true;
    private boolean cargoin = false;
    private boolean shippinghubnear = false;
    Orientation angles;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            telemetry.addData("BR power: ", rearRightPower);
            telemetry.addData("BL power: ", rearLeftPower);
    ...
    private void extrafunctions(){
        ...
        if (rb.shippingSensor.getDistance(DistanceUnit.CM) < 12)
            shippinghubnear = true;
        else
            shippinghubnear = false;
        if (rb.intakeSensor.getDistance(DistanceUnit.CM) < 11)
            cargoin = true;
        else
            cargoin = false;
        if (gamepad1.left_trigger > TRIGGER_THRESHOLD)
            slowModeMultiplier = .25;
        //else if (shippinghubnear && cargoin && rb.sliderSpool.getCurrentPosition() < -60)
          //  slowModeMultiplier = .25;
        else
        ...
        /*
        else if (gamepad1.right_trigger > TRIGGER_THRESHOLD || (shippinghubnear && cargoin)) {
            slowModeMultiplier = .25;
        }
...
//        blinkinLedDriver.setPattern(pattern);
        ...
        if (rightX < -DRIVE_STICK_THRESHOLD || rightX > DRIVE_STICK_THRESHOLD || leftY < -DRIVE_STICK_THRESHOLD || leftY > DRIVE_STICK_THRESHOLD || leftX < -DRIVE_STICK_THRESHOLD || leftX > DRIVE_STICK_THRESHOLD) {
            ...
            */
            ...
            double strafe = (gamepad1.left_stick_x) * 1.1 * slowModeMultiplier;
             ...
             */
            rb.RBmotor.setPower(rearRightPower);
            rb.RFmotor.setPower(frontRightPower);
            rb.LBmotor.setPower(rearLeftPower);
            rb.LFmotor.setPower(frontLeftPower);

            //rb.drivefour(frontRightPower * slowModeMultiplier, frontLeftPower* slowModeMultiplier, rearRightPower* slowModeMultiplier, rearLeftPower* slowModeMultiplier);
